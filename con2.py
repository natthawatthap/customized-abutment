...
# Function to generate model 3 based on the lower part of model 1 and the upper part of model 2
def generate_model3():
    # Extract the lower part of model 1 (mechs below model a)
    lower_part_model1 = original_vertices[0][original_vertices[0][:, 2] <= 0]

    # Extract the upper part of model 2 (mechs above model b)
    upper_part_model2 = original_vertices[1][original_vertices[1][:, 2] >= 0]

    # Concatenate the lower and upper parts to create model 3 vertices
    model3_vertices = np.vstack((lower_part_model1, upper_part_model2))

    # Discard excess vertices if the size is not divisible by 9
    excess_vertices = model3_vertices.shape[0] % 9
    if excess_vertices > 0:
        model3_vertices = model3_vertices[:-excess_vertices]

    # Reshape the model3_vertices array to the shape (n_triangles, 3, 3)
    n_triangles = model3_vertices.shape[0] // 9
    model3_vertices = np.reshape(model3_vertices, (n_triangles, 3, 3))

    # Create a mesh object for model 3
    model3_mesh = mesh.Mesh(np.zeros(n_triangles, dtype=mesh.Mesh.dtype))
    model3_mesh.vectors = model3_vertices

    # Plot model 3
    model3 = mplot3d.art3d.Poly3DCollection(model3_mesh.vectors)
    ax.add_collection3d(model3)
    models.append(model3)
    original_vertices.append(np.copy(model3_vertices))
    fig.canvas.draw_idle()

# Function to load and display the STL files
def load_stl_files(filenames):
    global models, original_vertices
